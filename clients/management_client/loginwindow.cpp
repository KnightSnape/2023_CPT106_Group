...
void LoginWindow::login() {
    ...
    qDebug("ping json %s", json.toStdString().c_str());
    ...
    operation["name"] = "query_permission";
    json_obj["operation"] = operation;
    doc = QJsonDocument(json_obj);
    json = doc.toJson(QJsonDocument::Compact).toStdString().c_str();
    qDebug("query json %s", json.toStdString().c_str());
    ...
    if(this->sock->waitForConnected(5000)) {
        ...
        if (state["succ"].toInt() == 1) {
            ...
            this->role = return_val["permission"].toString();
            this->close();
        } else {
            qDebug() << "server failed";
            QMessageBox::critical(this, "error", state["reason"].toString(), QMessageBox::Ok);
